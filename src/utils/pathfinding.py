    ...
    while cola and len(visitados) < max_profundidad:
        ...
        for dx, dy, direccion in direcciones:
            ...
            if (
                ...
                and es_valido_func(nx, ny)
                and not hay_obstaculo_func(nx, ny)
            ):
                visitados.add((nx, ny))
...
def a_star(
    ...
    while open_set:
        ...
        for dx, dy, direccion in direcciones:
            ...
            if not es_valido_func(neighbor[0], neighbor[1]) or hay_obstaculo_func(
            ...
            if neighbor not in g_score or tentative_g_score < g_score[neighbor]:
                ...
                f_score[neighbor] = tentative_g_score + heuristica_func(
                    neighbor, objetivo
                )
...
def dijkstra(
    inicio: Tuple[int, int], objetivo: Tuple[int, int], es_valido_func, hay_obstaculo_func
) -> Optional[List[str]]:
    if inicio == objetivo:
        return []
    ...
    came_from: Dict[Tuple[int, int], Optional[Tuple[Tuple[int, int], str]]] = {
        inicio: None
    }
